...
export const listAdminClaims = async (db: Firestore, bucket: Bucket): Promise<AdminClaimsListResponse> => {
  ...
  return { claims, total: claims.length, summary: buildClaimsSummary(claims) };
};

export const listClaimsForUser = async (db: Firestore, bucket: Bucket, uid: string): Promise<UserClaimsListResponse> => {
